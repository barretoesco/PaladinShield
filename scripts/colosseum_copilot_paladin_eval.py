...
def main() -> None:
    ...
    for row in (r1.get("results") or [])[:10]:
    ...
    for row in (r2.get("results") or [])[:8]:
        p = row.get("project") or row
        slug = p.get("slug")
        name = p.get("name")
        liner = (p.get("oneLiner") or "")[:130]
        print(f"  • [{slug}] {name}: {liner}")
    ...
    print("\nWrote:", out, "| /tmp/copilot_paladin_winners.json | /tmp/copilot_paladin_archives.json")
